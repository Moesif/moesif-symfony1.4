...
class SendCurlTaskConsumer extends SendTaskConsumer {
    ...
    protected function _execute_forked($url, $data) {
        ...
        $exec = ' echo \'' . $data . '\' '.' | '.'curl -v -X POST -H "Content-Type: application/json" -H "User-Agent: moesif-symfony/1.4" -H "X-Moesif-Application-Id: ' . $applicationId .'" -d @- "' . $url . '"';
